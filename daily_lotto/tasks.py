...
from lotto.celery import app

logger = get_task_logger(__name__)


@app.task
def create_lotto_task():
    ...
    # House share
    commission_prize = DailyLotto.TICKET_PRICE * ticket_count
    house_pool = DailyLotto.HOUSE_COMMISSION_RATE * commission_prize
    ...
    # Six share
    six_prize = DailyLotto.TICKET_PRICE * ticket_count
    six_prize_pool = DailyLotto.JACKPOT_SHARE_RATE * six_prize

    # Five share
    five_prize = DailyLotto.TICKET_PRICE * ticket_count
    five_prize_pool = DailyLotto.FIVE_SHARE_RATE * five_prize

    # Four share
    four_prize = DailyLotto.TICKET_PRICE * ticket_count
    four_prize_pool = DailyLotto.FOUR_SHARE_RATE * four_prize

    # Three share
    three_prize = DailyLotto.TICKET_PRICE * ticket_count
    three_prize_pool = DailyLotto.THREE_SHARE_RATE * three_prize
    ...
    current_lotto = DailyLotto.objects.get(start_date=datetime.date.today())
    current_lotto.jack_pot = jackpot
    current_lotto.save()
    ...
    for ticket in DailyLottoTicket.objects.filter(purchased_time__contains=datetime.date.today()):
        ...
        if len(list(matches)) == 6:
            ...
            DailyLotto.objects.filter(start_date__contains=datetime.date.today()).update(jack_pot=backup, backup_jackpot=0)
