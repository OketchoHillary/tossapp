...
from lotto_api.models import DailyLotto, DailyLottoTicket, DailyQuota, CommissionSum, now_plus_1, DailyLottoResult
...
def commission():
    ...
    previous_hse_com = DailyQuota.objects.filter(daily_lotto=daily_previous).values_list('house_commission')
...
def lotto_jackpot():
    ...
    current_sixQuota = DailyQuota.objects.filter(daily_lotto=daily).values_list('six_number_prize_pool')
...
def daily_draw():
    ...
    DailyLotto.objects.filter(lotto_id=daily.lotto_id).update(win1=num1, win2=num2, win3=num3, win4=num4, win5=num5, win6=num6)

    cur_ticket = 0
    for ticket in DailyLottoTicket.objects.filter(daily_lotto=daily):
        ...
        # creating winners
        DailyLottoResult.objects.create(daily_lotto=daily, hits_number_prize=matches_count, prize=0,
                                        daily_lotto_ticket=c2, winners=p2)

        # Commissions
        a = DailyLottoTicket.objects.filter(daily_lotto=daily, hits=3).count()
        b = DailyLottoTicket.objects.filter(daily_lotto=daily, hits=4).count()
        c = DailyLottoTicket.objects.filter(daily_lotto=daily, hits=5).count()
        d = DailyLottoTicket.objects.filter(daily_lotto=daily, hits=6).count()

        # backupJackpot
        bp = DailyLotto.objects.filter(lotto_type='D').values_list('backup_jackpot')[1]
        backup = int(bp[0])

        p6 = DailyLotto.objects.filter(lotto_type='D').values_list('jack_pot')[0]
        pool6 = int(p6[0])

        pool5 = DailyQuota.objects.filter(daily_lotto=daily).values_list('five_number_prize_pool')
        y = list(pool5[0])
        my_pool5 = y[0]

        pool4 = DailyQuota.objects.filter(daily_lotto=daily).values_list('four_number_prize_pool')
        z = list(pool4[0])
        my_pool4 = z[0]

        pool3 = DailyQuota.objects.filter(daily_lotto=daily).values_list('three_number_prize_pool')
        v = list(pool3[0])
        my_pool3 = v[0]
        ...
        if matches_count == 3:
            DailyQuota.objects.filter(daily_lotto=daily).update(three_number_prize_pool_commission=for3)
        ...
        if matches_count == 5:
            ...
            DailyLottoTicket.objects.filter(daily_lotto=daily, hits=5).update(ticket_prize=for5)
            Tuser.objects.filter(username=p2).update(balance=F("balance") + for5)
        if matches_count == 6:
            DailyQuota.objects.filter(daily_lotto=daily).update(six_number_prize_pool_commission=for6)
            DailyLottoResult.objects.filter(daily_lotto=daily, hits_number_prize=6).update(prize=for6)
