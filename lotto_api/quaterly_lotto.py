...
#
# from lotto_api.models import DailyLotto, DailyLottoTicket, DailyQuota, CommissionSum, DailyLottoResult, now_plus_2
# from tossapp.models import *
# from django.db.models import F
# from tauth.settings import NUMBER_RANGE
#
#
# def previous_q_lotto():
#     return DailyLotto.objects.filter(lotto_type='Q').order_by('-start_date')[1]
#
#
# def quaterly_lotto():
#     return DailyLotto.objects.filter(lotto_type='Q').order_by('-start_date')[0]
#
#
# ticket_count = DailyLottoTicket.objects.filter(daily_lotto=quaterly_lotto()).count()
#
# # daily revenue
...
#
# # current lotto id
# toss_lotto = quaterly_lotto()
# lottoid = toss_lotto.lotto_id
#
#
# # calculating ticket purchase commission
# def commission():
#     DailyQuota.objects.filter(daily_lotto=quaterly_lotto()).update(house_commission=house_pool,
...
#
#     previous_hse_com = DailyQuota.objects.filter(daily_lotto=previous_q_lotto()).values_list('house_commission')
...
# def lotto_jackpot():
#     # previous jackpot
#     my_p = DailyLotto.objects.filter(lotto_type='Q').values_list('jack_pot').order_by('-start_date')[1]
#     pjackpot = int(my_p[0])
#
...
#                                       end_date=timezone.now() + now_plus_2(), lotto_type='Q', )
#     return lotto
#
#
# def quaterly_draw():
...
#
#         # backupJackpot
#         bp = DailyLotto.objects.filter(lotto_type='Q').values_list('backup_jackpot').order_by('-lotto_id')[1]
#         backup = int(bp[0])
#
#         p6 = DailyLotto.objects.filter(lotto_type='Q').values_list('jack_pot').order_by('-lotto_id')[0]
#         pool6 = int(p6[0])
#
